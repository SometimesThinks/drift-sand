...
|       수단        | 설명                                                                       |
| :---------------: | -------------------------------------------------------------------------- |
| **시간 슬라이더** | **시/분/초 3개로 분리된 슬라이더**. 각 슬라이더에서 해당 단위를 개별 선택. |
...
- 슬라이더 범위: **시 0–24**, **분 0–59**, **초 0–59**.

### 2.3 입력 검증·오류 처리

- **유효 범위 밖 입력**: 슬라이더 입력이 범위를 벗어나는 경우 → **유효 구간으로 클램프** 후 적용.
...
- **24시간 초과 조합**: 시/분/초 슬라이더 조합으로 전체 시간이 24시간(86,400초)을 초과하면 **24:00:00으로 클램프** 후 적용. 단, `24:00:00`에서 분/초를 올리는 경우는 위의 **24:00:00 경계 처리**를 우선 적용.
...
| 화면          | 진입 조건                                | 주요 동작                                                             |
| ------------- | ---------------------------------------- | --------------------------------------------------------------------- |
| **설정/대기** | 앱 최초 실행, 리셋 후, 완료 후 리셋 선택 | 슬라이더로 시간 설정,`시작` → 실행 화면                               |
